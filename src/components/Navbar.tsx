  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-md">
      ...
      <div className="mx-auto flex max-w-7xl h-16 items-center justify-between px-4">
        ...
        {/* Right nav triggers */}
        <div className="flex items-center gap-4">
          {/* 1. Buyer Panel / Marketplace tab button - always visible to logged-in users */}
          {userProfile && (
            <button
              onClick={() => setCurrentTab("browse")}
              ...
              id="buyer-panel-navbar-btn"
            >
              <ShoppingBag className="h-4 w-4" />
              <span>Buyer Panel (Shop)</span>
            </button>
          )}

          {/* 2. Seller Panel tab button - always visible to logged-in users. 
              If user is buyer, it acts as Become a Seller trigger.
              If user is seller or admin, it routes directly to the seller dashboard tab. */}
          {userProfile && (
            <button
              onClick={() => {
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border transition cursor-pointer ${
                currentTab === "seller"
                  ? "bg-violet-50 border-violet-200 text-violet-700 font-bold shadow-xs"
                  : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
              id="seller-panel-navbar-btn"
            >
              <Store className="h-4 w-4 text-violet-500" />
              <span>
                {userProfile.role === "buyer" ? "Become a Seller" : "Seller Panel"}
              </span>
            </button>
          )}

          {/* 3. Admin Panel tab button - only visible if role is admin */}
